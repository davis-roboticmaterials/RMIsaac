...
// Experemental
std::string PybindApplication::getAllConfigForComponent(const std::string& node, const std::string& component) {

  auto maybe_json = app_->backend()->config_backend()->getAllForComponent(node, component);
  if (maybe_json == NULL) {
    return "{}";
  }
...
void InitPybindApplication(pybind11::module& m) {
  pybind11::class_<PybindApplication>(m, "PybindApplication")
      ...
      .def("get_all_config_for_component", &PybindApplication::getAllConfigForComponent)
